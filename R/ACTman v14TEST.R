...
ACTman <- function(workdir, myACTdevice, iwantsleepanalysis, plotactogram, tune){
...
for(i in 1:length(ACTdata.files)){
  ...
  if(myACTdevice == "MW8"){ # Device-specific Data Management
    ACTdata.1 <- read.csv(paste(ACTdata.files[i]), header = TRUE, stringsAsFactors = FALSE)
  ...
  source(file = "C:/Bibliotheek/Studie/PhD/Publishing/ACTman/R-part/IS Calculation Module v5.R",
         local = TRUE)
  r2 <- IS
...
ACTman(workdir = workdir, myACTdevice = myACTdevice, iwantsleepanalysis = FALSE, plotactogram = TRUE, tune = FALSE)
