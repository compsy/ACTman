...
# Other days
day2 <- slice(act_data, (day2start+1):(day2start+1440))
day3 <- slice(act_data, (day2start+1441):(day2start+(2*1440)))
day4 <- slice(act_data, (day2start+(2*1440)+1):(day2start+(3*1440)))
day5 <- slice(act_data, (day2start+(3*1440)+1):(day2start+(4*1440)))
day6 <- slice(act_data, (day2start+(4*1440)+1):(day2start+(5*1440)))
day7 <- slice(act_data, (day2start+(5*1440)+1):(day2start+(6*1440)))
day8 <- slice(act_data, (day2start+(6*1440)+1):(day2start+(7*1440)))
day9 <- slice(act_data, (day2start+(7*1440)+1):(day2start+(8*1440)))
day10 <- slice(act_data, (day2start+(8*1440)+1):(day2start+(9*1440)))
day11 <- slice(act_data, (day2start+(9*1440)+1):(day2start+(10*1440)))
day12 <- slice(act_data, (day2start+(10*1440)+1):(day2start+(11*1440)))
day13 <- slice(act_data, (day2start+(11*1440)+1):(day2start+(12*1440)))
day14 <- slice(act_data, (day2start+(12*1440)+1):(day2start+(13*1440)))
...
# Load data
# act_data <- read.table("431-BG00013.txt")
# colnames(act_data) <- c("Date", "Time", "Activity")

# Days_count <- nlevels(act_data$Date) # Return number of days

# day2_start <- which(act_data$Time == "00:00:00") # Start of day2 at midnight.
# day2.start <- day2_start[1] # To remove all other midnight startingpoints.
# day2start <- day2.start - 1
...
# # Use this starting point as a way to calculate days, make every day a seperate variable, and then plot everything after each other.
# temprow <- matrix(c(rep.int(0, length(day2start))), nrow=day1.rest, ncol=3) # create vector to add rest of day one to.
# newrow <- data.frame(temprow)
# colnames(newrow) <- colnames(day1)
# day1 <- rbind(newrow, day1)
...
# par(mar = c(1,4,1,4)) # Set margins so plots are closer together
# barplot(day1$Activity, ylim = ylimit)
# abline(v = day2start+75, lty = 2) # Add start line
# barplot(day2$Activity, ylim = ylimit)
# barplot(day3$Activity, ylim = ylimit)
# barplot(day4$Activity, ylim = ylimit)
# barplot(day5$Activity, ylim = ylimit)
