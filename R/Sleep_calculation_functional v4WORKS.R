...
setwd("C:/Bibliotheek/Studie/PhD/Publishing/ACTman/R-part/Actogram & Sleep analysis")

# Load data (example data: actigraphy file from MotionWatch8 worn by Nicolien Knapen)
data <- ACTdata.1.sub # Previously read.csv("NK_data.csv")
...
for(a in 1:nrow(data.sleeplog)){
  ...
  aaa <- mutate(aaa, score = (
    (lag(Activity..MW.counts., n = 1L, default = 0)/5) +
      (lag(Activity..MW.counts., n = 2L, default = 0)/25) +
      (lead(Activity..MW.counts., n = 1L, default = 0)/5) +
      (lead(Activity..MW.counts., n = 2L, default = 0)/25) +
      Activity..MW.counts.))
  ...
  aaa$sleep.chance <- (lead(aaa$epoch.sleep.chance, n = 1L) +
                                     lead(aaa$epoch.sleep.chance, n = 2L) +
                                     lead(aaa$epoch.sleep.chance, n = 3L) +
                                     lead(aaa$epoch.sleep.chance, n = 4L) +
                                     lead(aaa$epoch.sleep.chance, n = 5L) +
                                     lead(aaa$epoch.sleep.chance, n = 6L) +
                                     lead(aaa$epoch.sleep.chance, n = 7L) +
                                     lead(aaa$epoch.sleep.chance, n = 8L) +
                                     lead(aaa$epoch.sleep.chance, n = 9L) +
  ...
  ## Now create a function which returns first $Time after certain time (lights out in sleep log)
  sleep.end. <- aaa.sleeptime[which(aaa.sleeptime$wakeup.chance == 2 & lead(aaa.sleeptime$wakeup.chance > 2)),]
  ## First row now contains the start of sleep.
